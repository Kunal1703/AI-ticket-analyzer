"use client";

import { useActionState } from "react";

import { SecretReveal } from "@/components/SecretReveal";
import { SubmitButton } from "@/components/SubmitButton";
import { createApiKeyAction, type SecretState } from "@/lib/admin/actions";

const INPUT =
  "h-9 w-full rounded-md border border-neutral-300 bg-white px-2 text-sm dark:border-neutral-700 dark:bg-neutral-900";

export function ApiKeyCreateForm() {
  const [state, action] = useActionState<SecretState, FormData>(createApiKeyAction, {});
  return (
    <form action={action} className="space-y-3">
      {state.error ? (
        <p role="alert" className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-950 dark:text-red-300">
          {state.error}
        </p>
      ) : null}
      {state.secret ? <SecretReveal label="API key" secret={state.secret} /> : null}
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
        <label className="flex-1 text-sm">
          <span className="mb-1 block font-medium">Name</span>
          <input name="name" required placeholder="CI pipeline" className={INPUT} />
        </label>
        <label className="flex-1 text-sm">
          <span className="mb-1 block font-medium">Scopes</span>
          <input name="scopes" defaultValue="analyze" className={INPUT} />
        </label>
        <SubmitButton pendingLabel="Creating…">Create key</SubmitButton>
      </div>
    </form>
  );
}
